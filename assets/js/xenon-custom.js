	...
	$(document).ready(function()
	{
		...
		var Ajax = function () {
            $.getJSON ("../assets/config.json", function (data) {
                ...
                {
                    var txt1="<li>\
                                <a href=\"#"+ item.title +"\" class=\"smooth\">\
                                    <i class=\""+ item.icon +"\"></i>\
                ...
                $.each (data["main_menu"]["list"], function (i, item)
                {
                    ...
                    $.each (data["main_menu"]["list"][i]["data"], function (j, item)
                    {
                        var menu_ul_li_a = "<li>\
                                                <a href=\"#"+ item.title +"\" class=\"smooth\">\
                                                    <span class=\"title\">"+ item.title +"</span>\
                ...
                {
					var h4 = "<h4 class=\"text-gray\">\
							<i class=\"linecons-tag\" style=\"margin-right: 7px;\" id=\""+ item.h4_name +"\">\
							</i>"+ item.h4_name +"</h4>"
					$(".main-footer").before(h4)
					var row = null
					$.each (data["main_content"][i]["data"], function (j, item)
                	{	
						if(j % 4 == 0)
						{
						...
						row.append(col_sm_3)
						if (j % 4 == 0 || j == data["main_content"][i]["data"].length-1) {
							$(".main-footer").before(row)
						}
					});
					$(".main-footer").before("<br />")
                });
